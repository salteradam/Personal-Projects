...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, starting_game_state):
        ...
        "*** YOUR CODE HERE ***"
        # Our start state contains Pac-Mans position and a tuple of booleans that represent 
        # if a particular corner has been reached or not
        self.starting_state = (self.starting_position, (False, False, False, False))
    ...
    def is_goal_state(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Our tuple is represented by 4 boolean positions (False, False, False, False). 
        # When all boolean positions are true (all 4 corners have been reached) we have reached the goal state.
        return all(state[True]) 
    ...
    def get_successors(self, state):
        ...
        for action in [
           Directions.NORTH,
            ...
            # state now contains Pac-Mans position, and the status of corners visited
            (x, y), corners_visited = state
            dx, dy = Actions.direction_to_vector(action)
            nextx, nexty = int(x + dx), int(y + dy)

            if not self.walls[nextx][nexty]:
                # Since corners_visited is an immutable tuple represented in our state we need to convert it to a 
                # mutable list to update if a corner is reached
                new_corners_visited = list(corners_visited) 

                for i in range(len(self.corners)): # loop through the corners

                    if (nextx, nexty) == self.corners[i]: # check if Pac-Mans position is at 1 of the 4 corners
                        new_corners_visited[i] = True # Pac-Man is at a corner, update the boolean at position i (1 of the 4 corners)

                new_corners_visited = tuple(new_corners_visited) # change back to an immutable tuple to update our state 
                next_state = ((nextx, nexty), new_corners_visited) # update our state 
                cost = 1 # assuming cost of 1
...
def corners_heuristic(state, problem):
    ...
    unvisitedCorners = [] # List to store corners that we need to visit
    for i in range(len(corners)):
        if not cornersVisited[i]: # Corner hasn't been visited, add to list
            unvisitedCorners.append(corners[i])
   
    distanceToUnvisited = [] # List to store distances to unvisited corners
    # Get the distances from our position to a corner
    for cornerToVisit in unvisitedCorners:
        # Using Manhattan distance as our heuristic
        distance = abs(position[0] - cornerToVisit[0]) + abs(position[1] - cornerToVisit[1])
        distanceToUnvisited.append(distance)

    # A corner exists, get the max distance for the heuristic 
    if distanceToUnvisited:
        maxDistanceToUnvisited = max(distanceToUnvisited)
    else:
        maxDistanceToUnvisited = 0 # Heuristic should return 0 when done

    return maxDistanceToUnvisited
...
def food_heuristic(state, problem):
    ...
    foodList = food_grid.as_list() # Convert our food_grid to a list for iteration
    distanceToFood = [] # List that stores calculated distances
    for food in foodList:
        # Maze distance gives a better approximation for the heuristic than Manhattan distance
        # distance = abs(position[0] - food[0]) + abs(position[1] - food[1])
        distance = maze_distance(position, food, problem.starting_game_state)
        distanceToFood.append(distance)
        
    # List is not empty, find the furthest away food for our heuristic   
    if distanceToFood:
        maxDistanceToFood = max(distanceToFood)
    else:
        maxDistanceToFood = 0 # Heuristic should return 0 when done
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def find_path_to_closest_dot(self, game_state):
        ...
        "*** YOUR CODE HERE ***"
        # Do BFS on the problem (getting closest food)
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def is_goal_state(self, state):
        ...
        """
        x, y = state
        # The goal is to get to the next food source (True if food exists in the next space)
